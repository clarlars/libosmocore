...
static int parse_ss(const struct gsm48_hdr *hdr, uint16_t len, struct ss_request *req)
{
	...
	LOGP(0, LOGL_DEBUG, "msg_type: 0x%X\n", msg_type);
				  ...
				  struct ss_request *req, uint8_t msg_type)
{
	...
	LOGP(0, LOGL_DEBUG, "parse length.  iei_length: 0x%X iei: 0x%X, len: 0x%X\n", iei_length, iei, len);
	/* If the data does not fit, report an error */
	if (len - offset < iei_length) { //2
		LOGP(0, LOGL_DEBUG, "parse length error.  iei_length: 0x%X iei: 0x%X, len: 0x%X\n", iei_length, iei, len);
		return 0;
	}
			   ...
			   struct ss_request *req)
{
	...
	// This causes problems when there is more that one transaction in a ussd session
	if (length < 3) {
		LOGP(0, LOGL_DEBUG, "ss invoke length error\n");
		return 0;
	}
	LOGP(0, LOGL_DEBUG, "Length is good\n");
	...
	offset = invoke_data[1] + 2;		// ?Really
	req->invoke_id = invoke_data[2];

	/* look ahead once */
	if (offset + 1 > length) {
		LOGP(0, LOGL_DEBUG, "ss invoke length error 2\n");
		return 0;
	}
	LOGP(0, LOGL_DEBUG, "Second length is good\n");
				 ...
				 struct ss_request *req)
{
	...
	if (length < 8)
		return 0;
	LOGP(0, LOGL_DEBUG, "Processing length is good.  Sequence: 0x%X, string tag: 0x%X, dcs(0x0F): 0x%X, string tag: 0x%X\n",
		uss_req_data[0 + offset], uss_req_data[2 + offset], uss_req_data[4 + offset], uss_req_data[5 + offset]);
